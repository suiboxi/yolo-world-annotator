...
from yolo_world_annotator.core.yolo_format import serialize_yolo
...
class InferenceEngine(QObject):
    """Persistent worker; the detector remains resident between jobs."""
    ...
    def run_job(self, job: dict) -> None:
        ...
        try:
            ...
            device_request = str(job.get("device") or "auto")
            ...
            if (
                ...
                or device_request != self.device_request
            ):
                device_info = resolve_device(device_request)
                self.status_changed.emit(
                    f"正在将 YOLO-World 加载到 {device_info.description}，首次需稍候…"
                )
            ...
            self.model_ready.emit(
                str(self.model_path),
                getattr(self.detector, "device_description", device_request),
            )
            total = len(paths)
            ...
            for index, path in enumerate(paths, start=1):
                ...
                if index == 1 or index % 10 == 0 or index == total:
                    self.status_changed.emit(
                        f"{self.detector.device_description} 正在标注 {index}/{total}：{path.name}"
                    )
        ...
        except Exception as exc:  # surface model, device and image errors to the GUI
...
class AnnotatorWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("YOLO-World 自动标注器")
        ...
        self._show_device_state()

    @property
    def default_model_path(self) -> Path:
        return default_weights_dir() / "yolov8s-worldv2.pt"
    ...
    def _build_settings_panel(self) -> QScrollArea:
        ...
        model_group = QGroupBox("推理设备与 YOLO-World 模型")
        ...
        if initial_index < 0 and initial_device.startswith("cuda:"):
            self.device_combo.addItem(initial_device.upper(), initial_device)
            initial_index = self.device_combo.count() - 1
        self.device_combo.setCurrentIndex(max(0, initial_index))
        self.device_combo.currentIndexChanged.connect(self._show_device_state)
        model_form.addRow(
            self._parameter_label(
                "设备策略",
                "自动模式优先使用 CUDA；当前环境没有可用 CUDA 时自动改用 CPU。",
            ),
            self.device_combo,
        )
        ...
        model_form.addRow(
            self._parameter_label("当前设备", "CPU 使用 float32；CUDA 使用 float16。"),
            self.device_value,
        )
    ...
    def _show_device_state(self, _index: int | None = None) -> None:
        requested = str(self.device_combo.currentData() or "auto")
        try:
            device = resolve_device(requested)
            text = device.description
            self.device_value.setText(text)
            color = "#4bd37b" if device.use_half else "#6db7ff"
            self.device_value.setStyleSheet(f"color:{color};font-weight:700")
            self.gpu_label.setText(f"设备已就绪：{device.description}")
            if not self.busy:
                self.auto_current_button.setEnabled(True)
                self.auto_all_button.setEnabled(True)
        except DeviceSelectionError as exc:
            self.device_value.setText(str(exc))
            self.device_value.setStyleSheet("color:#ff6767;font-weight:700")
            self.gpu_label.setText("设备不可用")
    ...
    def open_folder(self, folder: Path, select_path: Path | None = None) -> None:
        try:
            ...
            saved_device = str(config.get("device", self.device_combo.currentData() or "auto"))
    ...
    def _start_inference(self, paths: list[Path]) -> None:
        if self.busy:
            return
        device_request = str(self.device_combo.currentData() or "auto")
        try:
            device_info = resolve_device(device_request)
        except DeviceSelectionError as exc:
            self._show_error(str(exc))
        ...
        elif device_info.torch_device == "cpu":
            self.status_label.setText("将使用 CPU 推理；兼容性更好，但速度通常慢于 CUDA。")
        self.save_classes()
        self.busy = True
        self.last_batch_preview_at = 0.0
        self.batch_processed = 0
        ...
        settings["device"] = device_request
    ...
    def _on_inference_batch(self, results: list[tuple]) -> None:
        ...
        if boxes:
            message = f"已完成 {index}/{total}：{path.name}，txt 已写入 {len(boxes)} 行"
        else:
            message = f"已完成 {index}/{total}：{path.name}，未检测到目标，txt 为空"
    ...
    def _inference_finished(self, cancelled: bool) -> None:
        ...
        self.status_label.setText(("批量任务已取消。" if cancelled else "自动标注完成。") + summary)
    ...
    def _set_busy(self, busy: bool) -> None:
        try:
            resolve_device(str(self.device_combo.currentData() or "auto"))
            device_available = True
        except DeviceSelectionError:
            device_available = False
        self.auto_current_button.setEnabled(not busy and device_available)
        self.auto_all_button.setEnabled(not busy and device_available)
    ...
    def closeEvent(self, event: QCloseEvent) -> None:  # noqa: N802
        if self.busy:
            answer = QMessageBox.question(self, "正在推理", "推理任务仍在运行，是否取消并退出？")
